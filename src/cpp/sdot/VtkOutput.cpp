...
void VtkOutput::add_triangle( std::array<Pt,3> pts, const std::map<std::string,VTF> &point_data, const std::map<std::string,TF> &cell_data ) {
    add_item( pts.data(), pts.size(), VtkTriangle, point_data, cell_data );
}

void VtkOutput::add_pyramid( std::array<Pt,5> pts, const std::map<std::string,VTF> &point_data, const std::map<std::string,TF> &cell_data ) {
    std::array<Pt,5> npts{ pts[ 0 ], pts[ 1 ], pts[ 3 ], pts[ 2 ], pts[ 4 ] };
    add_item( npts.data(), npts.size(), VtkPyramid, point_data, cell_data );
}

void VtkOutput::add_wedge( std::array<Pt,6> pts, const std::map<std::string,VTF> &point_data, const std::map<std::string,TF> &cell_data ) {
    add_item( pts.data(), pts.size(), VtkWedge, point_data, cell_data );
}

void VtkOutput::add_tetra( std::array<Pt,4> pts, const std::map<std::string,VTF> &point_data, const std::map<std::string,TF> &cell_data ) {
    add_item( pts.data(), pts.size(), VtkTetra, point_data, cell_data );
}

void VtkOutput::add_quad( std::array<Pt,4> pts, const std::map<std::string,VTF> &point_data, const std::map<std::string,TF> &cell_data ) {
    add_item( pts.data(), pts.size(), VtkQuad, point_data, cell_data );
}

void VtkOutput::add_edge( std::array<Pt, 2> pts, const std::map<std::string,VTF> &point_data, const std::map<std::string,TF> &cell_data ) {
...
void VtkOutput::add_hexa( std::array<Pt,8> pts, const std::map<std::string,VTF> &point_data, const std::map<std::string,TF> &cell_data ) {
